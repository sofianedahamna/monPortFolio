...
function createCookie(newCookie){
    ...
    document.cookie = `${encodeURIComponent(newCookie.name)}=${encodeURIComponent(newCookie.vale)};expires=${newCookie.expires.toUTCString()} `;
...
function createElements(cookies) {
    ...
    cookies.forEach(cookie => {
      ...
      listItem.innerHTML =  `
        ...
        <p>
          <span>Valeur</span>: ${decodeURIComponent(formatCookie[0])}
        </p>
        <button>X</button>
      `;
